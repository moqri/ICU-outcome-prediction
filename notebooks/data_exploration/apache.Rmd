...
```{r}
ApachiOutPut=df%>%select(c(apachescore,hospitaldischargestatus))%>%subset(apachescore!='',hospitaldischargestatus!='')
ApachiOutPut$hospitaldischargestatus=as.factor(ApachiOutPut$hospitaldischargestatus)
correlation <- cor(ApachiOutPut$apachescore,as.numeric(ApachiOutPut$hospitaldischargestatus))
correlation

ggplot(ApachiOutPut, aes( x=hospitaldischargestatus, y=apachescore)) + 
  geom_violin(trim=FALSE)+ geom_boxplot(width=0.1)+xlab("Hospital discharge status")+ylab("Apachi Score") 
...
DiseaseTable<-filter (DiseaseTable, total>2000)
...
BoxPlot$apacheadmissiondx[which(BoxPlot$apacheadmissiondx=="Hypovolemia (including dehydration, Do not include shock states)")]<-"Hypovolemia"
...
ggplot(BoxPlot, aes(x=apacheadmissiondx, y=apachescore, fill=unitdischargestatus)) +
    geom_boxplot()+theme(axis.text.x = element_text(angle = 90, hjust = 1, size=7,color="black"))+xlab("Admission diagnosis")+ylab("Apache Score")+guides(fill=guide_legend(title="Unit discharge status"))
```	
